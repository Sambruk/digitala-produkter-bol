...
[Rules for decisionmaking](#rules-for-decisionmaking)
...
These are the rules that we agreed on

* Suggestions from the participants are put into Github Pull-requests to later conduct a digital dialogue around. If no one has rejected the PR after a 14 day period it should be merged.
* In Github issues, problems/questions are added.
* All requests or issues that someone deems necessary to discuss must be taken up at meetings.
* Github's version management must be used. Version numbers are set according to 1.2.3 where "1" is a major release (not backwards compatible), "2" is an intermediate release (new functionality) and "3" minor fixes/corrections. Decisions on release at level "2" are made at a board meeting, "3" can be made digitally with a simple "thumbs-up" majority, at least 6 "thumbs-ups" are required for a digital decision